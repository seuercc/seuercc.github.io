/**
 * 原子服务API调用封装模块（UMD格式）
 * 作用：统一封装对window.ascfwebProxy.invokeJsApi的调用，对外暴露结构化的API方法
...
(function (globalObj, factory) {
    ...
    function invokeAtomicServiceApi(apiName, options = {}) {
        // 校验原子服务代理是否存在，避免调用报错
        if (!window?.ascfwebProxy?.invokeJsApi) {
            console.error("[原子服务API] window.ascfwebProxy.invokeJsApi 未定义，无法调用API：", apiName);
            return;
        }
        // 调用核心API
