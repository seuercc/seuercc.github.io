   const style = document.createElement('style');
    ...
    function gameCenterGetPostParams() {
        if (window.HiSpaceObject) {
            ...
            resultContainer.innerHTML += `<div class="suc">✅ 游戏中心 getPostParams succeed<div class="result-content">${formattedResult}</div></div>`;
        } else {
            console.log('no window.HiSpaceObject')
    ...
    async function musicGetUserInfo() {
        if (window.AOPJSInterface) {
            ...
            resultContainer.innerHTML += `<div class="suc">✅ 音乐 getUserInfo succeed<div class="result-content">${res3}</div></div>`;
        } else {
    ...
    async function himovieSignInAsync() {
        if (window.JsInterface) {
            ...
            resultContainer.innerHTML += `<div class="suc">✅ 视频 signInAsync succeed<div class="result-content">${h5SessionId}</div></div>`;
        } else {
    ...
    function themeGetParams() {
        if (window.JsInterface) {
            ...
            // 优化：JSON格式化+换行
            const formattedResult = JSON.stringify(result, null, 2).replace(/\n/g, '<br>');
            resultContainer.innerHTML += `<div class="suc">✅ 主题 getParams succeed<div class="result-content">${formattedResult}</div></div>`;
        } else {
            console.log('no window.JsInterface')
            ...
            if (window.walletTokenInfoJsInterface) {
                window.walletTokenInfoJsInterface.getAccessToken()
                    .then((session) => {
                        ...
                        resultContainer.innerHTML += `<div class="suc">✅ 钱包 getAccessToken succeed<div class="result-content">${session}</div></div>`;
    ...
    function mapGetUserInfo() {
        if (window.HmsMapsJsBridge) {
            window.HmsMapsJsBridge.onmessage = function (param) {
                ...
                resultContainer.innerHTML += `<div class="suc">✅ 地图 getUserInfo succeed<div class="result-content">${formattedResult}</div></div>`;
            };
